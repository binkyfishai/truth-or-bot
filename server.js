...
app.post('/api/game', async (req, res) => {
  try {
    ...
    console.log(`New game request - Difficulty: ${difficulty}, Model: ${model}, Timestamp: ${new Date().toISOString()}`);
    ...
    try {
      ...
      // Check if the AI-generated title is identical to the real article title
      if (fakeArticle.title === realArticle.title || fakeArticle.title.trim() === realArticle.title.trim()) {
        console.log("Titles are identical! Generating a completely new title.");
        
        // Generate a completely random title only if needed
        const newTitle = generateRandomTitle();
        console.log(`Generated new random title: "${newTitle}"`);
        
        fakeArticle.title = newTitle;
        fakeArticle.content = `# ${newTitle}\n\n${fakeArticle.content}`;
      } else {
        console.log(`Using AI-generated title: "${fakeArticle.title}" (different from real article)`);
      }

      // Verify titles are now different
      console.log(`Final titles - Real: "${realArticle.title}", Fake: "${fakeArticle.title}"`);
      console.log(`Are titles identical? ${fakeArticle.title === realArticle.title ? 'YES! This is a problem!' : 'No, they are different.'}`);
      ...
      // Create a simple fake article as fallback with a guaranteed different title
      fakeArticle = {
      ...
      // Verify titles are different (should always be true with generateRandomTitle)
